...
function CabinSection({ cabin, index, onWhatsApp }: { cabin: Cabin; index: number; onWhatsApp: (msg: string) => void }) {
  ...
  const content = cabin[language as Language];
  const descText = content.detailedDescription || content.description;
    ...
    >
      <div className="max-w-[1440px] mx-auto w-full px-4 md:px-0">
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-12 py-10 md:py-14">
          <h3 className="text-3xl md:text-5xl font-serif font-bold text-foreground mb-8">{content.title}</h3>
          <div className="flex flex-col lg:flex-row gap-10 lg:gap-16">
              ...
              <div className="flex flex-wrap gap-6 py-4 border-b border-border/40">
                ...
                <div className="flex items-center gap-2.5 text-primary">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-muted-foreground">{content.bedsDetail}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2.5 text-primary">
                  <div className="p-2 rounded-xl bg-primary/10">
            ...
            <div className="w-full lg:w-[340px] shrink-0">
              <ReservationCalculator cabin={{...cabin, title: content.title}} onWhatsApp={onWhatsApp} />
            </div>
          </div>
        </div>
      </div>
...
export default function IndexPage() {
  ...
  const [siteData, setSiteData] = useState<ContentData | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/content')
      ...
      .catch((err) => {
        ...
        setError("No se pudo cargar el contenido del sitio. Intenta recargar la página.");
      });
  }, []);
  ...
  if (error) {
    return <div className="min-h-screen flex items-center justify-center bg-background text-destructive text-center p-8">{error}</div>;
  }
  ...
  const { hero, cabins, exclusiveRental, tours } = siteData;
  const heroData = hero?.[language as Language] || {
    tag: "BIENVENIDO",
  ...
  // Safari image can be one of the exclusive rental images
  const safariImage = exclusiveRental?.images?.[1] || "/assets/safari-hero.jpg";
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative">
      ...
      <AdminBar
        ...
        isAdmin={isAdmin}
        onToggleAdmin={() => {
          /* Lógica opcional para salir de modo edición visual */
        }}
      />
      ...
      <section id="nuestras-cabanas" className="w-full bg-background relative z-10">
        ...
        <div className="flex flex-col w-full">
          {cabins
            // Filter out invalid cabins based on the Spanish title
            .filter(cabin => cabin.es.title && cabin.es.title !== "Sin nombre" && cabin.slug !== "sin-nombre")
            .map((cabin, index) => (
      ...
      <ExclusiveSection t={t} onWhatsApp={handleWhatsAppClick} exclusiveData={exclusiveRental} />
      ...
      <section id="experiencias" className="py-24 px-4 md:px-8 bg-background">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {tours.map((tour, idx) => {
              const tourContent = tour[language as Language];
              return (
              <div key={tour.id} className="group relative h-[400px] rounded-2xl overflow-hidden cursor-pointer shadow-md hover:shadow-2xl transition-all duration-500"
                   onClick={() => handleWhatsAppClick(`Hola, me gustaría reservar la experiencia: ${tourContent.title}`)}>
                <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors duration-500 z-10" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent z-10" />
                <img 
                  src={exclusiveRental?.images?.[idx % exclusiveRental.images.length] || tour.imageUrl} 
                ...
                <div className="absolute bottom-0 left-0 p-6 z-20 w-full transform group-hover:-translate-y-2 transition-transform duration-500">
                  <h3 className="text-xl font-serif font-bold text-white mb-2">{tourContent.title}</h3>
      ...
      <section className="flex flex-col md:flex-row w-full h-[600px] overflow-hidden">
        ...
        >
          ...
          <div className="relative z-20 text-center space-y-6 px-8">
            ...
            <div className="inline-block px-8 py-4 bg-white text-black font-bold uppercase tracking-widest text-sm rounded-lg shadow-xl md:transform md:translate-y-4 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 md:transition-all md:duration-500">
              {t.inventionem.cta}
            </div>
          </div>
        </a>
        ...
        >
          ...
          <div className="relative z-20 text-center space-y-6 px-8">
            ...
            <div className="inline-block px-8 py-4 bg-accent text-black font-bold uppercase tracking-widest text-sm rounded-lg shadow-xl md:transform md:translate-y-4 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 md:transition-all md:duration-500">
              {t.organicos.cta}
            </div>
          </div>
        </a>
